...
def gen(camera_source):
    ...
    # Decide whether to save: only when streaming from an uploaded file path (string), not from webcam (int 0)
    save_video = isinstance(camera_source, str)
    writer = None
    output_path = None

    if save_video:
        # Derive output file path with timestamp
        ts = datetime.now().strftime("%Y%m%d_%H%M%S")
        output_path = os.path.join(OUTPUT_FOLDER, f"annotated_{ts}.mp4")

        # Safely get size & fps
        width  = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH) or 0)
        height = int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT) or 0)
        fps    = cap.get(cv2.CAP_PROP_FPS)
        if not fps or fps <= 1:  # fallback if FPS isn't reported
            fps = 30.0
    ...
    while True:
        ...
        ret_jpg, buffer = cv2.imencode('.jpg', processed_frame)
        if not ret_jpg:
            continue
    ...
    if writer is not None:
        writer.release()
        # Optionally, you could log/print the output path for debugging:
